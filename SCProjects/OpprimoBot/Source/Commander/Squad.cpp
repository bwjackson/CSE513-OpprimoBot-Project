...
//Brandon added
int Squad::getSquadMaxSpread()
{
	return squadMaxSpread;
}

int Squad::getMoveState()
{
	return moveState;
}

void Squad::setMoveState(int newState)
{
	moveState = newState;
}

double Squad::getSpread()
{
	TilePosition center = this->getCenter();
	for (auto &a : agents)
		spread += center.getDistance(a->getUnit()->getTilePosition());
	if (this->getSize() > 0)
		spread /= this->getSize();
	return spread;
}