...
bool NavigationAgent::computePathfindingMove(BaseAgent* agent, TilePosition goal)
{
	...
	if (agent->getSquadID() >= 0)
	{
		...
		if (sq != NULL)
		{
			...
			/**Determine max allowable spread*/
			double maxSpread = MAX_SPREAD_BASE + sq->getSquadMaxSpread() + sq->getSize();
			//double minSpread = MIN_SPREAD_BASE + sq->getSquadMaxSpread();
			TilePosition waypoint = agent->getUnit()->getTilePosition();
			/**Set squad movement to converge state if spread out too far*/
			if (maxSpread < sq->getSpread())
			{
				sq->setMoveState(Squad::MSTATE_CONVERGE);
				double distToWaypoint = 10000;
				for (auto &a : sq->getMembers())
				{
					TilePosition tempPosition = a->getUnit()->getTilePosition();
					double tempDistance = checkpoint.getDistance(tempPosition);
					if (tempDistance < distToWaypoint)
					{
						waypoint = tempPosition;
						distToWaypoint = tempDistance;
					}
				}
			}
			/**Return to normal movement if condenced enough*/
			if (sq->getMoveState() == Squad::MSTATE_CONVERGE && maxSpread > sq->getSpread())
				sq->setMoveState(Squad::MSTATE_NORMAL);
			/**Change movement goal from original path to converge*/
			if (goal.x != -1 && sq->getMoveState() == Squad::MSTATE_CONVERGE)
			{
				moveToGoal(agent, waypoint, goal);
				return true;
			}
		}
...
bool NavigationAgent::moveToGoal(BaseAgent* agent,  TilePosition checkpoint, TilePosition goal)
{
	...
	if (!unit->isMoving())
	{
		if (squadID != -1)
		{
			...
			double maxSpread = MAX_SPREAD_BASE + sq->getSquadMaxSpread() + sq->getSize();
			//double minSpread = MIN_SPREAD_BASE + sq->getSquadMaxSpread();
			Position waypoint = unit->getPosition();
			if (maxSpread < sq->getSpread())
			{
				sq->setMoveState(Squad::MSTATE_CONVERGE);
				double distToWaypoint = 10000;
				for (auto &a : sq->getMembers())
				{
					Position tempPosition = a->getUnit()->getPosition();
					double tempDistance = toReach.getDistance(tempPosition);
					if (tempDistance < distToWaypoint)
					{
						waypoint = tempPosition;
						distToWaypoint = tempDistance;
			...
			if (sq->getMoveState() == Squad::MSTATE_CONVERGE && maxSpread > sq->getSpread())
				sq->setMoveState(Squad::MSTATE_NORMAL);
			if (sq->getMoveState() == Squad::MSTATE_CONVERGE)
				return unit->attack(waypoint);
		}
